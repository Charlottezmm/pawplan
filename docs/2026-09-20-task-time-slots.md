...
Today presents a compact chronological agenda with start/end times, readable cards and an active-task highlight. Card height follows content rather than duration; there are no empty hourly grid rows. The current task opens in view within the bounded, scrollable list on desktop and mobile. MCP clients with an older cached tool catalog must refresh/reconnect to discover the task-timing tools; `get_agent_guidance.taskTiming` describes their workflow and permission requirements.
